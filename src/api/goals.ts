...
export const goalsApi = {
  // POST : 챌린지 시작
  startRun: async (): Promise<GoalRunResponse> => {
  ...
  // POST : 목표 포기하기
  giveUpRun: async (runId: number): Promise<GoalRunResponse> => {
    const response = await fetch(`${API_BASE_URL}/progress/runs/${runId}/give-up`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-Device-Id": "test-uuid-1234",
